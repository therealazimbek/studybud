...
#     {'id': 2, 'name': "Explore SQL"},
#     {'id': 3, 'name': "Create portfolio with me"},
# ]
...
def room(request, pk):
    ...
    messages_ = room_.message_set.all()
    ...
    context = {'room': room_, 'messages_': messages_, 'participants': participants}
    return render(request, 'base/room.html', context)


def userProfile(request, pk):
    user = User.objects.get(id=pk)
    rooms_ = user.room_set.all()
    room_messages = user.message_set.all()
    topics = Topic.objects.all()
    context = {'user': user, 'rooms': rooms_, 'room_messages': room_messages, 'topics': topics}
...
def deleteRoom(request, pk):
    ...
    if request.method == 'POST':
        room_.delete()
        return redirect('home')
    return render(request, 'base/delete.html', {'obj': room_})
